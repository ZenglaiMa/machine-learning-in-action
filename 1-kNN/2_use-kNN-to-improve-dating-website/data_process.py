    ...
    with open(filename) as fp:
        ...
        for line in lines:
            ...
            if line[-1] == 'didntLike':
                ...
                labels.append(3)
            index += 1
    return normalization(feature_mat), labels


# 归一化 (X-X_min)/(X_max-X_min) 或 (X-X_mean)/X_std
def normalization(feature_mat):
    x_min = np.min(feature_mat, axis=0)
    x_max = np.max(feature_mat, axis=0)
    return (feature_mat - x_min) / (x_max - x_min)
    # return (feature_mat - np.mean(feature_mat, axis=0)) / np.std(feature_mat, axis=0)
